...
amount | BigDecimal | TRUE | The amount that going to withdraw or deposit (Accept non fraction number)
...
To get status of a transaction request, you can call this API. You may need to call this API few times until getting a final status (success / failed) or implment callback transaction.
