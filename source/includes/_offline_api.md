...
Offline APIs provide you possibility to withdraw and deposit money from OY offline channel such as Cash Recycle Machine (CRM) and convenience store
...
code | String(6) | Required OTP code for Deposit or Withdrawal Process
...
inactive_at | String(19) | Time that the otp code need to ber refreshed("yyyy-MM-dd HH:mm:ss.SSS")
expired_at | String(19) | Time that otp code will expired and no longer can't be used.("yyyy-MM-dd HH:mm:ss.SSS")
...
Use this API to refresh code that have been exceeded the interval active time 3 hours.
