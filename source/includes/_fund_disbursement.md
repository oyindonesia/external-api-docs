...
## Get Disbursement Status with Callback Option

```shell
curl -X GET https://partner.oyindonesia.com/api/status-callback?partner_tx_id=OY123456&send_callback=false -H 'content-type: application/json, accept: application/json, x-oy-username:myuser, x-api-key:7654321'
```
...
An endpoint to retrieve and/or re-send the latest callback status of a disbursement request. 

Please contact us and submit a callback URL if you need a callback status of a disbursement request.

### HTTPS Request
`GET BASE_URL/api/status-callback?partner_tx_id=OY123123&send_callback=false`

### Request Parameters

Parameter | Type | Required | Description
--------- | ---- | ---------- | -------- | -----------
partner_trx_id | String(255) | TRUE | A unique transaction ID which callback status to be checked
send_callback | Boolean | TRUE | A flag to indiciate if the status of a disbursement request need to be re-sent as a callback or not
