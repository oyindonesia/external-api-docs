...
---- | ---- | ---- | ------ | -------
partner_tx_id | String | TRUE | A unique transaction ID provided by partner. | A partner_tx_id that has been succesfully paid cannot be used anymore under the same username and only accepts alphanumerics. If empty then OY will generate automatically.
...
email | String | TRUE | The email addresses where the payment link will be sent to. | Up to 3 emails separated by ";"
phone_number | Numeric | TRUE | Phone number of the payer for a transaction. | Do not use special character (e.g. "+")
...
expiration | datetime | TRUE | To set the expiration of the payment link (yyyy-MM-dd HH:mm:ss) | Expiration date will be defaulted to 24 hours if it is not defined.
