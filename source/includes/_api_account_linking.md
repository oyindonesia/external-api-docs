...
|data	|String(255)	|Account linking data in Object `{partner_user_id: <partner_user_id>, channel_code: <channel_code>, linking_url: <linking_url>}`. The detail will be explained in the next section	|
...
|linking_url	|String(255)	|Linking URL to be accessed by user to complete the account linking process	|

## How we notify account linking result?
- The E-Wallet Account Linking Callback will be sent as a POST request (using JSON format) to your web hook. Please inform your callback web hook to our business representative or to partner@oyindonesia.com. (Note: We send the callback ONLY for a successful account linking)
