...
| trx_counter | Numeric     |  FALSE | 1/-1 | A transaction counter to limit number of transaction that can be receive by va number. Currently we only support single transaction (value = 1) in Payment Routing. This parameter will be used only if you use BANK_TRANSFER payment method and routed to VA. |
