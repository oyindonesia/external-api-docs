...
| trx_expiration_time | Date string; yyyy-mm-dd hh:mm:ss format     |  FALSE | 24 hours | Set expiration time of transaction. If empty use default 24h.  Min exp time is 1 hour for non-QRIS transactions. For QRIS transactions (`need_frontend: false, list_enable_payment_method: "QRIS", list_enable_sof: "QRIS"`), this value is limited to 1 minute - 1 hour after the request is sent, since the QRIS validity period is dynamic within the aforementioned limit. |
