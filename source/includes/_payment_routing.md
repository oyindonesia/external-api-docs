...
| list_enable_payment_method | String; comma separated     |  TRUE | - | To configure payment methods to be enabled in the payment method page; For example, if need_frontend is TRUE, you can fill it with VA,EWALLET,QRIS,CARDS. If need_frontend is FALSE only can accept VA, CARDS and QRIS (EWALLET is not allowed and will return an error if passed as a request parameter) |
| list_enable_sof | String; comma separated      |  TRUE | - | To configure list of source of fund (banks or ewallets) to be enabled in the payment method page; For example, if need_frontend is TRUE, you can fill it with 008,009,dana_ewallet,linkaja_ewallet. If need_frontend is FALSE, this parameter should be filled only with one bank code, "QRIS" for QRIS payment, and "CC_DC" for CARDS payment (will return an error otherwise) |
